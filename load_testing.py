...
from mapreduce_implementation import TextAnalyzer, create_kafka_consumer, consume_messages
...
def get_sample_data(size=1000):
    ...
    try:
        ...
        logger.error("Failed to load Kaggle dataset. Please ensure kagglehub is installed and configured.")
        raise

def run_kafka_producer(texts: List[str], topic: str = "text_data", batch_size: int = 100):
    """Run a Kafka producer to send messages."""
    kafka_server = os.environ.get('KAFKA_BOOTSTRAP_SERVERS', 'localhost:9092')
    logger.info(f"Starting Kafka producer to {kafka_server}, sending {len(texts)} messages to topic {topic}")
...
def run_batch_test(data_sizes: List[int], worker_counts: List[int]) -> Dict[str, Any]:
    ...
    for size in data_sizes:
        ...
        for worker_count in worker_counts:
            ...
            if worker_count > 1:
                par_results["efficiency"] = (par_results["speedup"] / worker_count) * 100
            
            all_results.append(par_results)
            logger.info(f"Parallel ({worker_count} workers) execution time: {par_time:.2f} seconds")
...
def run_stream_test(data_sizes: List[int], worker_counts: List[int], topic: str = "text_data") -> Dict[str, Any]:
    ...
    for size in data_sizes:
        ...
        if not run_kafka_producer(texts, topic):
            logger.error(f"Failed to send messages to Kafka. Skipping stream test with size {size}.")
            continue
        ...
        if not consumer:
            logger.error("Failed to create Kafka consumer. Skipping sequential stream test.")
            continue
        ...
        if len(stream_texts) == 0:
            logger.warning(f"No messages received from Kafka for size {size}. Check Kafka setup.")
            continue
        ...
        for worker_count in worker_counts:
            logger.info(f"Running parallel stream analysis with {worker_count} workers...")
            
            # Create Kafka consumer
            consumer = create_kafka_consumer(topic, group_id=f'mapreduce_parallel_{size}_{worker_count}')
            if not consumer:
                logger.error(f"Failed to create Kafka consumer. Skipping parallel stream test with {worker_count} workers.")
                continue
            ...
            if len(stream_texts) == 0:
                logger.warning(f"No messages received from Kafka for size {size} with {worker_count} workers. Check Kafka setup.")
                continue
            ...
            # Calculate metrics
            par_results["throughput"] = len(stream_texts) / par_time if par_time > 0 else 0
            par_results["speedup"] = seq_time / par_time if par_time > 0 else 0
            if worker_count > 1:
                par_results["efficiency"] = (par_results["speedup"] / worker_count) * 100
            
            all_results.append(par_results)
            logger.info(f"Parallel ({worker_count} workers) stream processing time: {par_time:.2f} seconds")
            if "speedup" in par_results:
                logger.info(f"Speedup: {par_results['speedup']:.2f}x")
                if "efficiency" in par_results:
                    logger.info(f"Efficiency: {par_results['efficiency']:.1f}%")
...
def plot_results(batch_results, stream_results, output_prefix="load_test"):
    ...
    if not batch_df.empty:
        plot_processing_type_results(batch_df, f"{output_prefix}_batch.png", "Batch Processing")
    
    # Plot stream results
    if stream_df is not None and not stream_df.empty:
        plot_processing_type_results(stream_df, f"{output_prefix}_stream.png", "Stream Processing")
...
def plot_processing_type_results(df, output_file, title_prefix):
    ...
    data_sizes = sorted(df["batch_size"].unique()) if "batch_size" in df.columns else sorted(df["stream_size"].unique())
    ...
    for analyzer_type in ["sequential", "parallel"]:
        for worker_count in worker_counts:
            ...
            subset = df[(df["analyzer_type"] == analyzer_type) & (df["num_workers"] == worker_count)]
            if len(subset) > 0:
    ...
    for worker_count in worker_counts:
        ...
        subset = df[(df["analyzer_type"] == "parallel") & (df["num_workers"] == worker_count)]
    ...
    if speedup_available:
        ...
        plt.text(0.5, 0.5, 'Speedup metrics not available', ha='center', va='center', fontsize=12)
        plt.title(f"{title_prefix}: Speedup vs Data Size")
        plt.xticks([])
        plt.yticks([])
    ...
    if efficiency_available:
        ...
        plt.text(0.5, 0.5, 'Efficiency metrics not available', ha='center', va='center', fontsize=12)
        plt.title(f"{title_prefix}: Parallel Efficiency vs Data Size")
        plt.xticks([])
        plt.yticks([])
...
def plot_batch_vs_stream(batch_df, stream_df, output_file):
    ...
    for b_size, s_size in common_sizes:
        b_subset = batch_df[(batch_df["analyzer_type"] == "sequential") & (batch_df["batch_size"] == b_size)]
        s_subset = stream_df[(stream_df["analyzer_type"] == "sequential") & (stream_df["stream_size"] == s_size)]
    ...
    if batch_times and stream_times:
        ...
        plt.bar([i - 0.2 for i in x], batch_times, width=0.4, label="Batch", color='blue')
        plt.bar([i + 0.2 for i in x], stream_times, width=0.4, label="Stream", color='green')
    ...
    for b_size, s_size in common_sizes:
        ...
        b_worker_counts = batch_df[(batch_df["analyzer_type"] == "parallel") & (batch_df["batch_size"] == b_size)]["num_workers"].unique()
        b_worker = min(b_worker_counts, key=lambda x: abs(x - target_workers)) if b_worker_counts.size > 0 else None
        
        # Find stream entry with closest worker count to target
        s_worker_counts = stream_df[(stream_df["analyzer_type"] == "parallel") & (stream_df["stream_size"] == s_size)]["num_workers"].unique()
        s_worker = min(s_worker_counts, key=lambda x: abs(x - target_workers)) if s_worker_counts.size > 0 else None
        
        if b_worker and s_worker:
            b_subset = batch_df[(batch_df["analyzer_type"] == "parallel") & 
                               (batch_df["batch_size"] == b_size) & 
                               (batch_df["num_workers"] == b_worker)]
            s_subset = stream_df[(stream_df["analyzer_type"] == "parallel") & 
    ...
    if batch_times and stream_times:
        x = range(len(batch_times))
        plt.bar([i - 0.2 for i in x], batch_times, width=0.4, label=f"Batch ({batch_workers[0]} workers)", color='blue')
        plt.bar([i + 0.2 for i in x], stream_times, width=0.4, label=f"Stream ({stream_workers[0]} workers)", color='green')
        plt.xticks(x, x_labels)
    ...
    if batch_throughput and stream_throughput:
        ...
        plt.bar([i - 0.2 for i in x], batch_throughput, width=0.4, label="Batch", color='blue')
        plt.bar([i + 0.2 for i in x], stream_throughput, width=0.4, label="Stream", color='green')
        plt.xticks(x, x_labels)
    ...
    if batch_throughput and stream_throughput:
        x = range(len(batch_throughput))
        plt.bar([i - 0.2 for i in x], batch_throughput, width=0.4, label=f"Batch ({batch_workers[0]} workers)", color='blue')
        plt.bar([i + 0.2 for i in x], stream_throughput, width=0.4, label=f"Stream ({stream_workers[0]} workers)", color='green')
        plt.xticks(x, x_labels)
...
def main():
    parser = argparse.ArgumentParser(description='Run load testing for MapReduce implementation')
    parser.add_argument('--mode', type=str, choices=['batch', 'stream', 'both'], default='both',
                        help='Testing mode (batch, stream, or both)')
    parser.add_argument('--data-sizes', type=str, default='10,50,100,500',
    ...
    parser.add_argument('--topic', type=str, default='text_data',
                        help='Kafka topic for stream processing tests')
    parser.add_argument('--output-prefix', type=str, default='load_test',
    ...
    if batch_results and stream_results:
        ...
        all_df.to_csv(f"{args.output_prefix}_all_results.csv", index=False)
        logger.info(f"Saved all results to {args.output_prefix}_all_results.csv")
