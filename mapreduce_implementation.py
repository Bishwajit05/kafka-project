...
def create_kafka_consumer(
    topic: str, group_id: str = "mapreduce_consumer"
        ...
        for i, message in enumerate(consumer):
            ...
            try:
                ...
                if isinstance(message.value, dict):
                    msg_text = message.value.get("text") or message.value.get(
                        "clean_text"
                    )
                    if msg_text:
...
def run_performance_test(
    ...
    if upload_to_s3 and s3_bucket:
        try:
            ...
            s3_uploader.upload_file(metrics_file, s3_bucket, s3_metrics_key)
...
def plot_comparison(
    ...
    if "speedup" in df.columns and any(~df["speedup"].isna()):
        plt.bar(
            df["analyzer_type"] + " (" + df["num_workers"].astype(str) + " workers)",
            df["speedup"],
            color="g",
        )
    ...
    if "throughput" in df.columns:
        ...
        if (
            max(throughputs) / (min(throughputs) + 0.001) > 10
        ):  # If big difference, use log scale
            plt.yscale("log")
        plt.grid(True, linestyle="--", alpha=0.7)
    else:
        plt.text(
    ...
    if "efficiency" in df.columns and any(~df["efficiency"].isna()):
        ...
        plt.text(
            0.5,
            0.5,
            "Efficiency/Latency metrics not available",
            ha="center",
            va="center",
            fontsize=12,
        )
    ...
    if upload_to_s3 and s3_bucket:
        try:
            ...
            # Upload plot to S3
            s3_plot_key = (
                f"{s3_prefix}/performance/{timestamp}_{os.path.basename(output_file)}"
            )
            s3_uploader.upload_plot(output_file, s3_bucket, s3_plot_key)
            logger.info(f"Uploaded plot to s3://{s3_bucket}/{s3_plot_key}")

            # Upload CSV to S3
            s3_csv_key = (
                f"{s3_prefix}/performance/{timestamp}_{os.path.basename(csv_file)}"
            )
            s3_uploader.upload_file(csv_file, s3_bucket, s3_csv_key)
...
def main():
    ...
    s3_bucket = os.getenv(
        "S3_BUCKET_NAME"
    )  # Set your S3 bucket name in environment variables
    kafka_topic = os.getenv(
        "KAFKA_TOPIC", "text_data"
    )  # Match producer's default topic
    ...
    if upload_to_s3:
        logger.info(
            f"S3 uploads are ENABLED. Files will be uploaded to s3://{s3_bucket}/{s3_prefix}"
        )
    else:
        logger.warning(
            "S3_BUCKET_NAME environment variable not set. S3 uploads are DISABLED."
        )
    ...
    try:
        # Consume messages
        logger.info(
            f"Consuming up to {max_messages} messages from Kafka topic '{kafka_topic}'..."
        )
        texts = consume_messages(consumer, max_messages)

        if not texts:
            logger.warning(
                "No messages received from Kafka. Using sample data instead."
            )
            texts = [
        ...
        if par_results.get("speedup"):
            logger.info(
                f"Achieved speedup: {par_results['speedup']:.2f}x with {par_results['num_workers']} workers"
            )
        ...
        if available_cores > 2:
            ...
            if (
                available_cores // 2 != par_analyzer.map_reduce.num_workers
                and available_cores // 2 > 1
            ):
            ...
            for worker_count in test_worker_counts:
                ...
                scaling_analyzer = TextAnalyzer(
                    sequential=False, num_workers=worker_count
                )
                ...
                logger.info(
                    f"Workers: {worker_count}, Speedup: {scaling_results.get('speedup', 0):.2f}x"
                )
