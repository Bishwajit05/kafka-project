...
class S3Uploader:
    def __init__(self):
        ...
        if not all(
            ...
            print(
                "⚠️ Warning: Some AWS credentials are missing from environment variables"
            )
        ...
        # Track uploads
        self.upload_history = {"files": [], "dataframes": [], "plots": [], "models": []}
    ...
    def upload_dataframe(self, df, bucket, s3_key):
        ...
        try:
            ...
            self.s3_client.put_object(
                Bucket=bucket, Key=s3_key, Body=csv_buffer.getvalue()
            )
    ...
    def upload_plot(self, fig, filename, bucket, s3_key=None):
        ...
        try:
            ...
            if success:
                self.upload_history["plots"].append(
                    f"s3://{bucket}/{s3_key or filename}"
                )
            return success
